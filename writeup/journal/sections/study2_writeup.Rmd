...
tidy_corrs_to_print <- print_tidy_corrs %>%
  ...
  select("rowname", contains("IAT"), 
         ...
         contains("Occupation"), contains("Labels")) %>%
  rename(" " = "rowname")

tidy_corrs_to_print_reordered <- tidy_corrs_to_print[c(8,9,6,1,2,4,5,7,3),] %>% select(` `, "Residualized Behavioral IAT", "Residualized Explicit Bias","Percent Women in STEM","Language IAT (Subtitles)", "Language IAT (Wikipedia)", "Prop. Gender-Neutral Labels", "Occupation Bias (Subtitles)","Occupation Bias (Wikipedia)", "Median Country Age")
...
GN_OB_S <- text_tidy_corrs %>%
  filter(rowname ==  "Prop. Gender-Neutral Labels", 
...
(Subtitle corpus: `r B_OB_S`; Wikipedia corpus: `r B_OB_W`), and remained reliable after partialling out the effect of median country age (Subtitle corpus: `r B_OB_S_partial`; Wikipedia corpus: `r B_OB_W_partial`; Figure 3).  To examine the relative predictive power of gender bias in language statistics and proportion form overlap, we fit a linear regression predicting behavioral gender bias with both measures, controlling for median country age. [NEITHER IS PREDICTIVE - try mediation model?]. Bias in language statistics did not reliably predict explicit gender bias.


```{r, fig.pos = "!t!", fig.height = 3.5, fig.width = 4, fig.cap = "Residualized behavioral IAT gender bias as a function of the proportion of gender-netural labels for set of words referring to occupations. Error bands indicate standard error of the model estimate.", include = F}
