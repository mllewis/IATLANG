...
grammar_data <- read_csv(EMPIRICAL_GRAMMAR_PATH) %>%
  ...
  mutate(empirical_binary_grammatical_gender = ifelse(empirical_grammatical_gender_score > 0,
                                                      "empirically_gendered",
                                                      "empirically_not_gendered"))
...
ggplot(ridge_df, aes(y = empirical_binary_grammatical_gender,  
             x = es_iat_sex_age_order_implicit_resid,
             color = empirical_binary_grammatical_gender,
             fill = empirical_binary_grammatical_gender)) +
  ggridges::geom_density_ridges(alpha = .4, scale = .9, quantile_lines = TRUE, quantiles = 2)  +
  scale_color_manual(values = c("blue", "red")) +
  scale_fill_manual(values = c("blue", "red")) +
  #geom_pointrange(ci_df, aes(y = empirical_binary_grammatical_gender, x= mean, 
  #                           xmin = ci_lower, xmax= ci_upper)) +
  geom_text(aes(label = language_code, 
                y = jitter(as.numeric(as.factor(empirical_binary_grammatical_gender))+ .2, 1.1)
                ), 
        color = "black", size = 4) +
  theme_classic() +
  ggtitle("Behavioral Gender Bias and Grammar Type") +
  xlab("Residualized Behavioral IAT Gender Bias") +
  ylab("Empirical Grammar Type \n(gender marking of IAT person words)") +
  theme(legend.position = "none",
        text = element_text(size = 12),
        axis.text.y = element_text(angle = 90, hjust=0.5, size = 12))
  
ggplot(ridge_df, aes(y = wikipedia_grammar_type2,  
             x = es_iat_sex_age_order_implicit_resid,
  ...
  #                           xmin = ci_lower, xmax= ci_upper)) +
  geom_text(aes(label = language_code, 
                y = jitter(as.numeric(as.factor(wikipedia_grammar_type2))+ .2, 1.1)
                ), 
        color = "black", size = 4) +
  theme_classic() +
  ggtitle("Behavioral Gender Bias and Grammar Type") +
  xlab("Residualized Behavioral IAT Gender Bias") +
  ylab("Theoretical Grammar Type \n(all morphosyntax)") +
  theme(legend.position = "none",
        text = element_text(size = 12),
        axis.text.y = element_text(angle = 90, hjust=0.5, size = 12))
```
