...
To understand the relative predictive power of gender bias in language statistics and proportion distinct forms, we fit an additive linear model predicting implicit behavioral gender bias with both measures, controlling for median country age. Neither bias in language statistics (Wikipedia: $\beta$ =  `r filter(wiki_pretty_text, term == "wiki_occu_semantics_fm") %>% pull(pretty_model)`; Subtitle: $\beta$ = `r filter(subt_pretty_text, term == "subt_occu_semantics_fm") %>% pull(pretty_model)`) nor proportion distinct forms (Wikipedia: $\beta$ =  `r filter(wiki_pretty_text, term == "mean_prop_distinct_occs") %>% pull(pretty_model)`; Subtitle: $\beta$ = `r filter(subt_pretty_text, term == "mean_prop_distinct_occs") %>% pull(pretty_model)`)  predicted implicit bias, likely due to collinearity between the two measures (Wikipedia: _r_ = `r wiki_corr`, _p_ < .001; Subtitle: _r_ = `r subt_corr`, _p_ < .001). One possible explanation for this pattern is that the effect of the proportion overlap in distinct forms on behavioral bias is mediated by language statistics. This causal model predicts that having distinct forms to refer to people of different genders _leads to_ biased language statistics. Consistent with this model, a bootstrap test of mediation revealed a significant mediation effect for the Wikipedia model (path-ab = `r wiki_ab`, _p_ = `r wiki_p`) and a marginal effect for the Subtitle model [path-ab = `r subt_ab`, _p_ = `r subt_p`; @robmed].\footnote{Though our power to detect this effect is relatively low (\~\ .4; Schoemann, Boulton, \& Short, 2017).}
...
Here are the models with language IAT as a predictor instead of occupation embeddings. When you control for median country age (when I hadn't done in the model I sent you), iat language isn't predictive. I don't think these models add anything to the narrative.

```{r} 
print(wiki_pretty_text_iat %>% select(-pretty_model))
print(subt_pretty_text_iat %>% select(-pretty_model))
```
