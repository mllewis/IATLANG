...
```{r, fig.pos = "!t!", fig.height = 4, fig.width = 4, fig.cap = "Residualized implicit psychological gender bias as a function of the proportion of gender-specific labels for set of words referring to occupations. Error band indicates standard error of the linear model estimate."}
...
ggplot(all_es_tidy2, aes(x = mean_prop_distinct_occs,
  ...
  geom_point(size = 1) +
  theme_classic() +
  ggtitle("Psychological Gender Bias and \nLexicalized Gender") +
  ylab("Implicit Psychological Gender Bias (residualized)\n") +
  xlab("Proportion Gender-Specific Occupation Labels") +
...
```{r, fig.height = 3.5, fig.cap = "Residualized psychological IAT gender bias as a function of mean gender bias of words referring to occupations, with each point corresponding to a language (Study 2). Occupation gender bias is estimated for each language from word embedding models trained on Subtitle (left) and Wikipedia (right) corpora. Error bands indicate standard error of the linear model estimate."}
...
all_es_tidy2 %>%
  ...
  mutate(model = fct_recode(model, "Subtitle Embeddings" = "subt_occu_semantics_fm",
    ...
    ylab("Implicit Psychological Gender Bias\n(residualized)\n") +
    xlab("\nOccupation Gender Bias from Language Statistics") +
    theme_classic() 
```
...
Finally, we examined the relationship between gender bias in language statistics and  psychological gender biases at the level of languages. Unlike in Study 1, all the target words in the present study referred to people (occupations) and thus potentially could be  marked for the gender of the referenced person. Consequently, if explicit gender marking drives language statistics, we should expect to see a strong positive relationship at the level of languages between bias in language statistics _for occupation words_ and psychological biases for speakers of that language. Consistent with this prediction, gender bias in language statistics for occupation words was positively correlated with implicit psychological gender bias (Subtitle: `r B_OB_S`; Wikipedia: `r B_OB_W`), and remained reliable after partialling out the effect of median country age (Subtitle: `r B_OB_S_partial`; Wikipedia: `r B_OB_W_partial`; Fig. 4). There was no relationship between language statistics for occupation words and explicit psychological gender bias, even after partialling out the effect of median country age (Subtitle: `r B_OE_S_partial`; Wikipedia: `r B_OE_W_partial`).

```{r}
all_es_tidy2_scaled <- all_es_tidy2 %>%
  mutate_if(is.numeric, scale)

wiki_regression <- lm(es_iat_sex_age_order_implicit_resid ~ mean_prop_distinct_occs + 
     wiki_occu_semantics_fm +  
     median_country_age, 
   data = all_es_tidy2_scaled) 

wiki_pretty_text <- wiki_regression %>%
  tidy() %>%
  rename(Beta = estimate, SE = std.error, Z = statistic, p = p.value) %>%
  mutate_if(is.numeric, ~ round(.,2)) %>%
  mutate(p = ifelse(p == 0, "<.0001", p),
         p = ifelse(p == "<.0001", p,  paste0("= ", p)),
...
# subt
subt_regression <- lm(es_iat_sex_age_order_implicit_resid ~ mean_prop_distinct_occs + 
     subt_occu_semantics_fm +  
     median_country_age, 
   data = all_es_tidy2_scaled)

subt_pretty_text <- subt_regression %>%
  tidy() %>%
  rename(Beta = estimate, SE = std.error, Z = statistic, p = p.value) %>%
  mutate_if(is.numeric, ~ round(.,2)) %>%
  mutate(p = ifelse(p == 0, "<.0001", p),
...
#car::vif(subt_regression)
```

To understand the relative predictive power of language statistics and distinct forms, we fit an additive linear model predicting implicit psychological bias from language statistics and proportion distinct forms, controlling for median country age. <!-- This order of reporting the results is a bit of a downer.. We know the predictors are highly collinear. What about using a residualized predictor from the outset? [ML] statistics are not reliable after residualizing out distinct forms. We could just not include any regression at all and skip right to the mediation analysis, since it's independently motivated and the predictors are colinear. -->
Neither bias in language statistics (Wikipedia: $\beta$ =  `r filter(wiki_pretty_text, term == "wiki_occu_semantics_fm") %>% pull(pretty_model)`; Subtitle: $\beta$ = `r filter(subt_pretty_text, term == "subt_occu_semantics_fm") %>% pull(pretty_model)`) nor proportion distinct forms (Wikipedia: $\beta$ =  `r filter(wiki_pretty_text, term == "mean_prop_distinct_occs") %>% pull(pretty_model)`; Subtitle: $\beta$ = `r filter(subt_pretty_text, term == "mean_prop_distinct_occs") %>% pull(pretty_model)`)  predicted implicit bias, likely due to collinearity between the two measures (Wikipedia: _r_ = `r wiki_corr`, _p_ < .001; Subtitle: _r_ = `r subt_corr`, _p_ < .001; VIF >= 2 for language statistics and proportion distinct forms in both Subtitle and Wikipedia models). The high degree of collinearity between language statistics and proportion distinct forms is consistent with a causal model in which  language statistics mediate the effect of distinct gender forms on implicit psychological bias: The presence of distinct forms referring to people of different genders _leads to_ biased language statistics, which in turn leads to gender bias in behavior. Consistent with this model, a bootstrap test of mediation revealed  a marginal effect for the Subtitle model [path-ab = `r subt_ab`, _p_ = `r subt_p`; @robmed], and significant mediation effect for the Wikipedia model (path-ab = `r wiki_ab`, _p_ = `r wiki_p`).\footnote{Though our power to detect this effect is relatively low (approximately, .4; Schoemann, Boulton, \& Short, 2017).}


```{r, include = F, eval = F}
# residualized analysis
wiki_residual_model <- lm(wiki_occu_semantics_fm ~ mean_prop_distinct_occs, all_es_tidy2_scaled)
subt_residual_model <- lm(subt_occu_semantics_fm ~ mean_prop_distinct_occs, all_es_tidy2_scaled)

all_es_tidy2_scaled_with_residuals <- all_es_tidy2_scaled %>%
  add_residuals(wiki_residual_model, "wiki_residuals") %>%
  add_residuals(subt_residual_model, "subt_residuals") 

lm(es_iat_sex_age_order_implicit_resid~ wiki_residuals + median_country_age, data = all_es_tidy2_scaled_with_residuals) %>%
  summary()

lm(es_iat_sex_age_order_implicit_resid~ subt_residuals + median_country_age, data = all_es_tidy2_scaled_with_residuals) %>%
  summary()

wiki_regression_iat <- lm(es_iat_sex_age_order_implicit_resid ~ mean_prop_distinct_occs + 
     lang_es_wiki +  
     median_country_age, 
   data = all_es_tidy2_scaled) 


wiki_pretty_text_iat <- wiki_regression_iat %>%
  tidy() %>%
  rename(Beta = estimate, SE = std.error, Z = statistic, p = p.value) %>%
  mutate_if(is.numeric, ~ round(.,2)) %>%
  mutate(p = ifelse(p == 0, "<.0001", p),
...
# subt
subt_regression_iat <- lm(es_iat_sex_age_order_implicit_resid ~ mean_prop_distinct_occs + 
     lang_es_sub +  
     median_country_age, 
   data = all_es_tidy2_scaled)

subt_pretty_text_iat <- subt_regression_iat %>%
  tidy() %>%
  rename(Beta = estimate, SE = std.error, Z = statistic, p = p.value) %>%
  mutate_if(is.numeric, ~ round(.,2)) %>%
  mutate(p = ifelse(p == 0, "<.0001", p),
...
In Study 2, we asked whether relatively "fossilized" features of language -- the presence of a grammatical gender systems and the propensity to lexicalize gender distinctions -- predicted degree of psychological gender bias in speakers of that language. Notably, we do not find a relationship between the presence of a grammatical gender system in a language and psychological gender bias. We do, however, find a relationship between the degree of lexicalized gender marking -- specifically, occupations -- and implicit psychological gender bias: Languages that have more gender-specific occupation terms tend to have speakers with greater psychological gender bias. Furthermore, we find some evidence to suggest that the effect of lexical gender distinctions on psychological gender bias may be mediated by biased language statistics for those words. In sum, this study provides  support for the language-as-causal-factor hypothesis, and suggests that the relationship between language and psychological biases may be specific to uses of languages that refer to people (rather than system-wide gender distinctions).
