...
study2_plot1 <- ggplot(all_es_tidy2, aes(x = mean_prop_distinct_occs,
  ...
  geom_text_repel(aes(label = language_name), size = 2.7) +
  ...
  ggtitle("Implicit Career-Gender Association\nand Occupation Terms") +
  ylab("Implicit Career-Gender Associaiton\n(residualized effect size)") +
...
by_word_df <- inner_join(bias_by_word, distinct_by_word, by = c("language_code", "occupation")) %>%
  mutate_if(is.numeric, scale) %>%
  filter(language_code != "zu") # exclude zulu as in study 1b 

# Subtitles model
sub_model <- lmer(abs(gender_diff_score_fm) ~ mean_distinct + (mean_distinct|language_code),
...
# Wiki model
wiki_model <- lmer(abs(gender_diff_score_fm) ~ mean_distinct + (mean_distinct|language_code),
...
GN_OB_S <- cor.test(all_corr_vars$mean_prop_distinct_occs, 
                    all_corr_vars$subt_occu_semantics_fm) %>%
  tidy() %>%
  tidy_r_to_text_r(.)

GN_OB_W <- cor.test(all_corr_vars$mean_prop_distinct_occs, 
                    all_corr_vars$wiki_occu_semantics_fm) %>%
  tidy() %>%
  tidy_r_to_text_r(.)
```
...
corr_text_df2 <- tidy_corrs %>%
  ...
  mutate(simple_r = paste0("r = ", f_num(simple_r, 2)),
         x = .06, y = -.07) %>%
  filter(model == "Wikipedia Embeddings")
...
study2_plot2 <- all_es_tidy2 %>%
  ...
  gather("model", "occ_lang_es", -language_name,
         -es_iat_sex_age_order_implicit_resid, -n_participants) %>%
  filter(model == "wiki_occu_semantics_fm") %>%
  mutate(model = fct_recode(model, "Subtitle Embeddings" = "subt_occu_semantics_fm",
    ...
    geom_point(alpha = .2) + 
    geom_text(data = corr_text_df2, aes(label = simple_r), x = .07, y = -.07, 
            color = "red", size = 5) +
...
pdf(P3_PATH, width = 10.25, height = 4.5)
