...
ggplot(all_ratings, aes(y = (8-maleness_norm), 
  ...
  ylab("Human Judgement of Gender Bias") +
...
```{r, fig.pos = "t", fig.height = 4, fig.cap = "V2: Implicit gender bias (adjusted for age, sex, and block order) as a function of the linguistic gender bias derived from word-embeddings (Study 1b). Each point corresponds to a language. Linguistic biases are estimated from models trained on text in each language from Subtitle (left) and Wikipedia (right) corpora. Larger values indicate a larger bias to associate men with the concept of career and women with the concept of family. Error bands indicate standard error of the linear model estimate.",  eval = F, include = F}
...
df_for_plotting2 <- all_es_tidy_participant %>%
  ...
  mutate(model = fct_recode(model, "Subtitle Embeddings" = "lang_es_sub",
                                   "Wikipedia Embeddings" = "lang_es_wiki"))  %>%
  filter(model ==  "Subtitle Embeddings")


  
# n participants log
  ggplot(df_for_plotting1, aes(x = lang_es,
                               y = es_iat_sex_age_order_implicit_resid,)) +
  geom_point(aes(size = log(n_participants)), alpha = .6) + 
...
# violin plots
  ggplot(df_for_plotting2, aes(x = lang_es, y = es_iat_sex_age_order_implicit_resid)) +
  geom_violin(aes(x = lang_es, y = es_iat_sex_age_order_implicit_resid, group = language_name), size = .7) +
  geom_smooth(method = "lm") +
...
df_for_plotting2_cis <- df_for_plotting2 %>%
  ...
  multi_boot_standard(col = "es_iat_sex_age_order_implicit_resid")
...
## Study 1c: A pre-registered study of biases in British and American English
In Study 1c, we conducted a confirmatory, pre-registred analysis of our hypothesis that biases present in language statistics are reflected in psychological biases of speakers of those languages. To do this, we leveraged an exisiting dataset from the Attitiudes, Identities, and Individual Differences Study [AIID; @aiid] containing measures of IAT performance from over 200,000 participants for a wide range of  IAT types (e.g. career - family, team - individual, etc.). Because most participants in this sample were English speakers, we compared biases between participants who spoke two dialects of English: British and American. For each of the 31 IAT types in the sample, we predicted that the degree to which that bias was present in a speaker's English dialect (British or American) would predict the magnitude of their psychological bias, as measured by the IAT. 
...
The AIID datset was partioned into two samples: exploratory (15%) and confirmatory (85%). Based on the exploratory sample, we pre-registed our analysis plan for the conformatory sample (https://osf.io/3f9ed). We note where are analysis diverges from the preregistration plan below. 

Of the 95 IAT types present in the dataset, we identified 31 types, based on the following criteria: (1) stimuli were words rather than pictures, and (2) 75% of the target words for each IAT test were present in both the BNC and COCA models. To measure the bias in language, we trained word embedding models on comparably-sized corpora of British [British National Corpus; @burnard1995users] and American [Corpus of Contemporary American English; @davies2008corpus] English (see SM for details). We then calculated a language bias effect size for each IAT in each English dialect, using the same method as in Study 1b. 

There were 187,969 IATs in the confirmatory AIID dataset from participants in the USA or UK. Our exclusion procedure for the behavioral data was similiar to Study 1a above (see SM for details). Our final sample included data `r format(nrow(full_1c_df), big.mark = ",")` IATs (USA: _N_ = `r format(pull(final_participant_counts,us), big.mark = ",") `; UK: _N_ = `r format(pull(final_participant_counts,uk), big.mark = ",") `).  Each participant completed an average of `r pull(n_iats_per_participant, mean)` administrations of the IAT (_SD_ = `r pull(n_iats_per_participant, sd)`). For each administrations of the IAT, we calculated a residual IAT score, controlling for  sex, age, education, task order (relative ordering of implicit versus explicit measures), and block order (relative ordering of congruent versus incongruent mappings on IAT task).  

We fit a linear mixed effect model predicting the magnitude of the bias for each participant with language dialect (American or British English), magnitude of the  bias difference between the two dialects, and an interaction betwen these two terms as fixed effected. We included participant and domain as random intercepts. This model differs from the pre-registered analysis, which is also consistent with this analysis, but failed to account for participant-level variance; see SM). 

## Results
Table 2 shows the model parameters. As predicted, we find that that degree of bias in language is reliable predictor of psychological bias across a range of IAT types. 
...
study_1c_model <- lmer(behavioral_effect_resid ~ residence*lang_diff + (1|user_id) + (1|domain), 
     data = full_1c_tidy)

papeR::prettify(summary(study_1c_model), confint = F) %>%
 # kable()
  kable("latex", digits = 2, booktabs = T, 
      caption = "Model parameters for Study 1c")  %>%
