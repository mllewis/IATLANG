...
p1 <- ggplot(all_ratings, aes(y = (8-maleness_norm), 
 ...
 # ggtitle("Gender Bias of English Words") +
  ...
  annotate("text", y = 2, x = .1, label = 
             paste0("italic(r) == ", f_num(sub_gender_score_corr$estimate, 2)), 
...
```{r plot1, eval = F}
P1_PATH <- here("writeup/journal/figs/p1.pdf")

pdf(P1_PATH, width = 3.46457, height  = 3)
p1
dev.off()
```

Estimates of gender bias from the Subtitle corpus (_M_ = `r round(mean(all_ratings$maleness_embedding_sub),2)`; _SD_=  `r round(sd(all_ratings$maleness_embedding_sub),2)`) and  the Wikipedia corpus (_M_ = `r round(mean(all_ratings$maleness_embedding_wiki),2)`; _SD_ =  `r round(sd(all_ratings$maleness_embedding_wiki),2)`) were highly correlated with each other (`r tidy_r_to_text_r(model_gender_score_corr)`). Critically, bias estimates from both word embedding models were also highly correlated with human judgements (_M_ = `r round(mean(all_ratings$maleness_norm),2)`; _SD_ =  `r round(sd(all_ratings$maleness_norm),2)`; _r_~Subtitle~ = `r tidy_r_to_text_r(sub_gender_score_corr)`; _r_~Wikipedia~ = `r tidy_r_to_text_r(wiki_gender_score_corr)`; Fig. 1). This suggests that the psychological gender bias of a word can be reasonably estimated from word embeddings. 
...
all_es <- left_join(iat_behavioral_es, iat_lang_es, by = "language_code") %>%
  ...
  select(language_code,language_name,family,n_participants,es_iat_sex_age_order_implicit_resid, es_iat_sex_age_order_explicit_resid,
         median_country_age, per_women_stem_2012_2017, lang_es_sub, lang_es_wiki, mean_prop_distinct_occs, subt_gender_diff_score_fm_abs,
...
all_corr_vars <- all_es_tidy %>%
  select(lang_es_sub, lang_es_wiki,subt_gender_diff_score_fm_abs,
         wiki_gender_diff_score_fm_abs, mean_prop_distinct_occs,
         es_iat_sex_age_order_explicit_resid, 
         es_iat_sex_age_order_implicit_resid, 
         per_women_stem_2012_2017, median_country_age) %>%
  rename(`Residualized Implicit Bias (IAT)` = "es_iat_sex_age_order_implicit_resid",
          ...
          `Occupation Bias (Subtitle)` = "subt_gender_diff_score_fm_abs",
          `Occupation Bias (Wikipedia)` = "wiki_gender_diff_score_fm_abs",
...
corpus_effect_cohens_d <- effsize::cohen.d(
                  all_es_tidy %>% 
                  filter(!is.na(lang_es_sub)) %>%
                   pull(lang_es_sub),
                 all_es_tidy %>% 
                  filter(!is.na(lang_es_sub)) %>%
                  pull(lang_es_wiki)) 
...
```{r, table2}
all_vars_names <- names(all_corr_vars)

crossed_vars <- cross_df(list(var1 = all_vars_names, var2 = all_vars_names)) %>%
  filter(var1 != var2)

get_nested_data <- function(var1_name, var2_name, df){
   all_corr_vars %>%
    select(var1_name, var2_name) %>%
      rename(var1 = var1_name,
            var2 = var2_name) %>%
    mutate(variable1_name = var1_name,
           variable2_name = var2_name) %>%
    select(variable1_name, variable2_name, var1, var2)
}

get_table_text <- function(df) {
  
  corr_df <- cor.test(df$var1, df$var2) %>%
    tidy() %>%
    select(estimate, p.value, conf.low, conf.high) %>%
    mutate_at(vars(estimate, conf.low, conf.high), ~round(.x, 2)) %>%
    mutate(p_value = case_when(p.value < .001 ~ "<.001", TRUE ~ as.character(round(p.value, 2))))
  
    paste0(
      pull(corr_df, estimate),
      " ", 
      " [",
      pull(corr_df, conf.low),
      ", ",
      pull(corr_df, conf.high),
      "](",
      pull(corr_df, p_value),
      ")"
    )
...
corrs_for_table <- map2_df(crossed_vars$var1, crossed_vars$var2, get_nested_data, all_corr_vars) %>%
      group_by(variable1_name, variable2_name) %>%
      nest() %>%
      mutate(r_table_text = map_chr(data, get_table_text)) %>%
      select(-data)

```

```{r bigtable}
tidy_corrs_to_print_simple <- corrs_for_table %>%
  spread(variable2_name, r_table_text)  %>%
  select(contains("Residualized"), contains("STEM"), contains("IAT"),
         contains("Occupation Labels"), contains("Occupation Bias"),
         contains("Age")) %>%
  rename(" " = "variable1_name") %>%
  ungroup()

tidy_corrs_to_print_reordered <- tidy_corrs_to_print_simple[c(8,9,6,1,2,7,4,5,3),] %>%
  slice(-1)
...
participant_level_data <- read_csv(BEHAVIORAL_P_IAT_PATH,
                                   col_names = c("country_code",	"country_name",	
                                                 "sex",	"log_age",	"education",
                                                 "overall_iat_D_score",	"order",
                                                 "explicit_dif",
                                                 "es_iat_sex_age_order_explicit_resid",
                                                 "es_iat_sex_age_order_implicit_resid"))
...
corr_text_df <- tidy_corrs %>%
    filter(rowname == "Language IAT (Wikipedia)", 
           var2 == "Residualized Implicit Bias (IAT)") %>%
    mutate(model = fct_recode(var2,"Wikipedia Embeddings" = "Language IAT (Wikipedia)"))  %>%
    select(model, simple_r) %>%
    mutate(simple_r = paste0("italic(r) == ", f_num(simple_r, 2))) %>%
    mutate(x = .85, y = -.07)

# plot lang vs behavioral
study1_plot1 <- all_es_tidy %>%
  select(language_name, lang_es_sub, lang_es_wiki,
         es_iat_sex_age_order_implicit_resid, n_participants) %>%
  gather("model", "lang_es", -language_name,
         -es_iat_sex_age_order_implicit_resid, -n_participants) %>%
  filter(model == "lang_es_wiki") %>%
  mutate(model = fct_recode(model, "Subtitle Embeddings" = "lang_es_sub",
                                   "Wikipedia Embeddings" = "lang_es_wiki")) %>%
  ...
  annotate("text", y = -.065, x = .75, label = 
              corr_text_df$simple_r, 
              color = "red", size = 2.6, parse = T) +
...
L_B_sub <- cor.test(all_corr_vars$"Language IAT (Subtitle)", 
                    all_corr_vars$"Residualized Implicit Bias (IAT)") %>%
  tidy() %>%
  tidy_r_to_text_r(.)

L_B_wiki <- cor.test(all_corr_vars$"Language IAT (Wikipedia)", 
                    all_corr_vars$"Residualized Implicit Bias (IAT)") %>%
  tidy() %>%
  tidy_r_to_text_r(.)
...
observations <- full_1c_df %>%
  distinct(user_id, domain) %>%
  nrow()

final_administration_counts <- full_1c_df %>%
  distinct(user_id, domain, residence) %>%
  count( residence) %>%
  spread(residence, n)

n_iats_per_participant <- full_1c_df %>%
  distinct(user_id, residence, domain) %>%
  count(user_id) %>%
...
```{r 1cplot,  fig.height = 4, fig.width = 4, set.cap.width=T, num.cols.cap=1, fig.cap = "Difference (British - American) in implicit bias (y-axis) and linguistic bias (x-axis) for each of the 31 IAT types in our dataset."}
...
study1_plot2 <- ggplot(es_iat_tidy, aes(x = lang_diff, y = behavioral_resid_diff)) +
  geom_smooth(method = "lm", alpha = .1) + 
  ggrepel::geom_text_repel(aes(label = domain2), size = 1.2,  
...
```{r plot2, eval = F}
P2_PATH <- here("writeup/journal/figs/p2.pdf")

pdf(P2_PATH, width = 7.08661, height = 3.1)
cowplot::plot_grid(study1_plot1, study1_plot2, ncol = 2,
                             labels = c('a','b'), label_size = 12, rel_widths = c(1.3,1))
dev.off()
```
