...
pdf(P1_PATH, width = 4.5, height = 4.)
p1
dev.off()
```
...
study1_plot1 <- all_es_tidy %>%
  ...
  geom_text(data = corr_text_df, aes(label = simple_r), x = .75, y = -.065, 
            color = "red", size = 5) +
...
study1_plot2 <- ggplot(es_iat_tidy, aes(x = lang_diff, y = behavioral_resid_diff)) +
  ...
  ggrepel::geom_text_repel(aes(label = domain2), size = 2.2) +
  geom_point() +
...
pdf(P2_PATH, width = 10.25, height = 4.5)
cowplot::plot_grid(study1_plot1, study1_plot2, ncol = 2,
                             labels = c('A','B'), label_size = 16, rel_widths = c(1.3,1))
dev.off()
```
