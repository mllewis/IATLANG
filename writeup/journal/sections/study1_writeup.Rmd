...
# fix croatian to be mean of hr and sr and exclude zulu
...
all_es <- left_join(iat_behavioral_es, iat_lang_es, by = "language_code") %>%
  mutate(lang_es_wiki = case_when(language_code == "hr" ~ hr_new,
                                  TRUE ~ lang_es_wiki)) %>%
  filter(language_code != "zu") # too many missing words from model (12/32)

lang_family_tidy <- lang_family %>%
  filter(wiki_language_code != "zu") 
```

For each country represented in our analysis of the Project Implicit, we identified the most frequently spoken language in each country using Ethnologue [@simons2018]. This included a total of `r nrow(all_es) + 1` unique languages. For one language, Zulu, over a third of the target words (12 out of 32) were missing from the model (see below), and so we excluded this language from our analysis. Our final sample included `r nrow(all_es)` languages, representing `r  length(unique(lang_family_tidy$family))` different language families.
...
t_test_data_corpus_effect <- all_es %>%
  filter(!is.na(lang_es_sub), !is.na(lang_es_wiki))

corpus_effect_t_test <- t.test(t_test_data_corpus_effect$lang_es_sub, 
       t_test_data_corpus_effect$lang_es_wiki, paired = T)
```
