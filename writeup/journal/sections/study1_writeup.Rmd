...
```{r, eval = F, include = F}
all_es2 <- all_es %>%
  filter(!(language_code %in% c( "id", "el", "ro", "ru", "ar", "pl", "zu", "he", "tr", "es")))
implicit_corr <- cor.test(all_es2$es_iat_sex_age_order_implicit_resid,
                              all_es2$lang_es_wiki) %>%
  tidy()

implicit_controlled <- lm(es_iat_sex_age_order_implicit_resid ~ lang_es_sub+ median_country_age, data = all_es2) %>%
  tidy()
...
ggplot(aes(es_iat_sex_age_order_implicit_resid, n_participants_scaled)) +
  geom_point() +
  geom_smooth(method = "lm")
...
lm(es_iat_sex_age_order_explicit_resid ~ lang_bias + median_country_age, data = sub, weights = file_size_scaled) %>%
  summary()

lm(es_iat_sex_age_order_implicit_resid ~ lang_bias + median_country_age, data = wiki, weights = mean_weight) %>%
  summary()
...
lm(es_iat_sex_age_order_implicit_resid ~ lang_bias , data = sub,  weights = file_size_scaled) %>%
  summary()
...
wtd.cor(x = wiki$es_iat_sex_age_order_implicit_resid, 
        y = wiki$lang_bias, 
        weight = wiki$file_size_scaled)

lm(es_iat_sex_age_order_implicit_resid ~ lang_bias + median_country_age, data = wiki, weights = file_size_scaled) %>%
  summary()
