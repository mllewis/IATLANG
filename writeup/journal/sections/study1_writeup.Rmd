...
lmer_model_string <- study_1c_model %>% 
  filter(term == "residenceus:lang_diff") %>%
...
There was a reliable interaction between language dialect and the magnitude of the  bias difference between the two dialects ($\beta$ `r lmer_model_string`), suggesting that language bias was a reliable predictor of implicit bias. Figure 3 shows the difference in bias (British - American) in language and behavior for each of the 31 IATs in our dataset. 
...
ggplot(es_iat_tidy, aes(x = lang_diff, y = behavioral_resid_diff)) +
  ggrepel::geom_text_repel(aes(label = domain), size = 2) +
  geom_point() +
  geom_smooth(method = "lm", alpha = .1) + 
  ylab("Implicit Bias Difference (residualized)") +
  xlab("Language Bias Difference (effect size)") +
