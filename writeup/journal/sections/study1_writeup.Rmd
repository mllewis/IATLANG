...
pdf("/Users/mollylewis/Documents/research/Grants/faculty_applications/TALK/figs/iatfig1.pdf", width = 5.5, height = 5)

ggplot(all_ratings, aes(y = maleness_norm, x = maleness_embedding_sub)) +
  geom_point(size = 1, alpha = .1) +
  ggtitle("Gender Association of English Words") +
  ylab("Human Judgement") +
  xlab("Word Embedding Estimate") +
...
pdf("/Users/mollylewis/Documents/research/Grants/faculty_applications/TALK/figs/iatfig2.pdf", width = 5.5, height = 5.5)
all_es_tidy %>%
  select(language_code, lang_es_sub, lang_es_wiki,
         es_iat_sex_age_order_implicit_resid, n_participants) %>%
  gather("model", "lang_es", -language_code,
 ...
 #                                  "Wikipedia Embeddings" = #"lang_es_wiki")) %>%
  ...
  geom_smooth(method = "lm", alpha = .1, size = .9) +
  ggrepel::geom_text_repel(aes(label = language_code), 
                           size = 5, box.padding = 0.1) + 
  geom_point(size = .7) + #aes(size  = log(n_participants))) + #
  theme_minimal() +
  ggtitle("Behavioral and Linguistic \nGender Biases") +
  ylab("Behavioral IAT Bias \n(residualized)") +
  xlab("Linguistic Bias\n (effect size)") +
