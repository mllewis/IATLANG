...
```{r}
implicit_corr <- cor.test(all_es_tidy$es_iat_sex_age_order_implicit_resid,
                              all_es_tidy$lang_es_wiki) %>%
  tidy()

implicit_controlled <- lm(es_iat_sex_age_order_implicit_resid ~ lang_es_wiki + median_country_age, data = all_es_tidy) %>%
  tidy()
...
explicit_corr <- cor.test(all_es_tidy$es_iat_sex_age_order_explicit_resid,
                              all_es_tidy$lang_es_wiki) %>%
  tidy()
```

There was no overall difference in the estimates of gender bias between models trained on the subtitle corpora versus the Wikipedia corpora (_t_(`r pluck(corpus_effect_t_test$parameter, "df")`) =  `r pluck(corpus_effect_t_test$statistic, "t")`, _p_ =  `r corpus_effect_t_test$p.value`). We next asked about the relationship between  estimates of gender bias for each language and implicit gender bias of participants from countries where that language was dominant (and, we assume, was the native language of most of these individuals). Language gender bias and implicit gender bias were positively correlated (_r_ = `r round(implicit_corr$estimate, 2)`, _p_ =  `r round(implicit_corr$p.value, 2)`; Fig. 2). This relationship held when controlling for median country age in a linear model ($\beta$= `r lang_beta`; _SE_ = `r lang_se`, _p_ = `r lang_p`). Linguistic gender bias was not correlated with explicit gender bias (_r_ = `r round(explicit_corr$estimate, 2)`; _p_ =  `r round(explicit_corr$p.value, 2)`). Table 1 shows the language-level correlations between all variables. [do we want to weight by model size?]
...
all_corr_vars <- all_es_tidy %>%
  select(lang_es_sub, lang_es_wiki, es_iat_sex_age_order_explicit_resid, 
         es_iat_sex_age_order_implicit_resid, per_women_stem, median_country_age, ggi_stoet) %>%
...
simple_corr <- psych::corr.test(all_corr_vars)$r %>%
  as_tibble(rownames = "rowname") %>%
  gather("var2", "simple_r", -rowname)
  
simple_corr_p <-  psych::corr.test(all_corr_vars)$p %>%
  as_tibble(rownames = "rowname") %>%
  gather("var2", "simple_p", -rowname)
  
partial_psych_obj <- psych::partial.r(data = all_corr_vars, x = 1:5, y = 6) 
partial_corr <- psych::corr.p(partial_psych_obj, n = nrow(all_corr_vars) - 1)$r %>%
  psych_to_mat() %>%
  as_tibble(rownames = "rowname") %>%
  gather("var2", "partial_r", -rowname)

partial_corr_p <- psych::corr.p(partial_psych_obj, n = nrow(all_corr_vars) - 1)$p %>%
  psych_to_mat() %>%
  as_tibble(rownames = "rowname") %>%
...
kable(tidy_corrs_to_print_reordered, "latex", booktabs = T, escape = F,
      caption = "Correlation (Pearson's r) for all measures in Study 1 at the level of languages. Numbers in parentheses show partial correlations controlling for median country age. Single astericks indicate p < .05 and double astericks indicate p < .01.",
      align = "lrrrrrr") %>%
...
```{r, eval = F}
# exploring weighting by file size
tidy_language <- all_es_tidy %>%
  select(language_code, lang_es_sub, lang_es_wiki,
...
lm(es_iat_sex_age_order_implicit_resid ~ lang_bias  + median_country_age, data = sub) %>%
  summary()
...
lm(es_iat_sex_age_order_implicit_resid ~ lang_bias  + median_country_age, data = wiki) %>%
  summary()
