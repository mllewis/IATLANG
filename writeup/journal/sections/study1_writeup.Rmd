...
all_corr_vars <- all_es_tidy %>%
  ...
  mutate(subt_occu_semantics_fm = abs(subt_occu_semantics_fm), ### TEMP!!!
         wiki_occu_semantics_fm = abs(wiki_occu_semantics_fm)) %>%
...
study1_plot1 <- all_es_tidy %>%
  ...
  scale_x_continuous(breaks = c(-.15, -0,.25, .5,.75, .9), 
                     label = c("male-\nfamily", "0", ".25", ".5", ".75", "male-\ncareer") , limits = c(-.2, .9)) +
  scale_y_continuous(breaks = c(-.075, -.05, -.025, 0, .025, .05), 
                     label = c("-.075\weaker", "-.05", "-.025", "0", ".025", ".05\stronger") , limits = c(-.08, .06) ) +
  scale_size(trans = "log10", labels = scales::comma, name = "N participants") +
  geom_text(data = corr_text_df, aes(label = simple_r), x = .7, y = -.075, 
            color = "red", size = 5) +
  ggtitle("Psychological and Linguistic\nCareer-Gender Association") +
...
study1_plot2 <- ggplot(es_iat_tidy, aes(x = lang_diff, y = behavioral_resid_diff)) +
  ...
  geom_point() +
  ylab("Implicit Gender Association Difference\n(residualized effect size)") +
  xlab("Language Gender Association Difference\n(effect size)") +
 ...
 scale_y_continuous(breaks = c( -.2,-.1, 0, .1, .2, .3), 
  ...
  ggtitle("Psychological and Linguistic Gender\nAssociations in British and American\nParticipants") +
  theme_classic(base_size = 12) +
  theme(axis.line = element_line(size = .8))
