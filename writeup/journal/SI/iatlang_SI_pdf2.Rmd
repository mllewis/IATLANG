---
title: "What are we learning from language? Associations between gender stereotypes and distributional structure in 25 languages"
...
This document was created from an R markdown file.  The respository for the project can be found here: https://github.com/mllewis/IATLANG/.


####  NOTE: The SI is intended to be viewed interactively online at https://mollylewis.shinyapps.io/iatlang_SI/.
...
```{r, fig.cap = "Note: Data from the US are excluded from this plot because of the large number of participants (_N_ =  638,082)."}
country_iat %>%
...
#### NOTE: See online version for this content (https://mollylewis.shinyapps.io/iatlang_SI/)
...
```{r, echo = FALSE, cache = F, fig.height = 9}
HISTO_DATA_PATH <- "data/DV_histo_data.csv"
histo_data <- read_csv(HISTO_DATA_PATH)

histo_data %>%
  filter(measure %in% c("overall_iat_D_score", "es_iat_sex_age_order_implicit_resid")) %>%
  ...
  ggplot(aes(x = bin, y = n, fill = measure)) +
  geom_bar(stat = "identity", width = .2) +
  facet_wrap(~country_name, scales = "free_y", ncol = 5)  +
    ggtitle("Implicit IAT data by country") +
...
histo_data %>%
  filter(measure %in% c("explicit_dif", 
                        "es_iat_sex_age_order_explicit_resid")) %>%
   mutate(measure = fct_recode(measure, 
                        Residualized = "es_iat_sex_age_order_explicit_resid",
                        Raw = "explicit_dif"),
        measure = fct_rev(measure),
        bin2 = round(bin, 0)) %>% # this is because binwidths are different for two measures
  ggplot(aes(x = bin2, y = n, fill = measure)) +
    geom_bar(stat = "identity", width = 1) +
    facet_wrap(~country_name, scales = "free_y", ncol = 5)  +
    ggtitle("Explicit IAT data by country") +
    xlab("Score") +
    theme(
      strip.text = element_text(size = 10),
      text = element_text(size = 10),
...
full_world +
  annotation_custom(grob = ggplotGrob(europe_inset),
                    xmin = 60, xmax = 180, ymin = 50, ymax = 130)
```
...
# converted using https://tableconvert.com/
...
Here we replicate the original set of Caliskan, Bryson, and Narayanan (2017; henceforth _CBN_) findings using the models trained on the corpora used in our paper, English Wikipedia (Bojanowski, Grave, Joulin, & Mikolov, 2016) and Subtitles (Lison & Tiedemann, 2016; Van Paridon & Thompson, 2019).
...
Partial correlation (Pearson's _r_) for all measures in Study 1 and 2 at the level of languages, controlling for median country age. Single asterisks indicate _p_ < .05 and double asterisks indicate _p_ < .01. The + symbol indicates a marginally significant _p_-value, _p_ < .1.
...
kable(cached_corr_table[c(12:16,18:20),c(-7, -10, -11, -12)],  booktabs = T, escape = F, # partial corrs only
      align = "l") %>% 
...
The table below shows the correlation (Pearson’s _r_) for all measures in Study 1 and 2 at the level of languages, including estimates of language bias obtained from models trained on the untranslated Wikipedia corpus.  

```{r}
cached_corr_table2 <- cached_corr_table %>%
  slice(1:11) %>%
  rename("Lang. Occup. Genderness (Wikipedia, untranslated)" = "Lang. Occup. Genderness\n(Wikipedia, untranslated)",
         "Male-Career Assoc. (Wikipedia, untranslated)" = "Male-Career Assoc.\n(Wikipedia, untranslated)")

kable(cached_corr_table2,  booktabs = T, escape = F, # full corrs only
      align = "l") %>% 
...
The pre-registration to this study can be found here: https://osf.io/3f9ed.

## IAT Target Words

Presented below are the target words for each of the 31 IATs used in our analyses. Each IAT has two target categories (e.g. "money" and "love") and two attribute categories (e.g. negative and positive). 
...
other_data <- read_csv(OTHER_PATH) %>%
  ...
  left_join(all_lang_model_scaled %>% 
...
We also find that per capita GDP (World Bank database; 2017 GDP per capita (current US$ indicator) is correlated with linguistic gender bias measured in both corpora (Wikipedia: `r implicit_gdp_wiki`; Subtitle: `r implicit_gdp_subt`). Futher, the magnitude of the male-career association in the language spoken in a country predicts the magnitude of that bias measured via the behavioral IAT, controlling for both national GDP and median country age. Results are presented below for mixed-effect models predicting behavioral IAT at the country level with median country age, GDP, and Language IAT as fixed effects, and random intercepts by language.

<b> Wikipedia Corpus: </b>
```{r}
wiki_gdp_model <- lmer(es_iat_sex_age_order_implicit_resid ~ lang_es_wiki +  
                         gdp_2017 + median_country_age + (1|language_code), 
                 data = other_data)  
...
wiki_gdp_model %>%
  ...
  mutate(term = c("(Intercept)", 
                  "Lang. Male-Career Assoc (Wikipedia)", 
                  "GDP", 
                  "Median Country Age")) %>%
  kable() 
```
...
```{r}
subt_gdp_model <- lmer(es_iat_sex_age_order_implicit_resid ~ lang_es_sub + 
                         gdp_2017 + median_country_age + (1|language_code), 
                 data = other_data)  
...
subt_gdp_model %>%
  ...
  mutate_if(is.numeric, round, 2) %>%
  mutate(term = c("(Intercept)", 
                  "Lang. Male-Career Assoc (Subtitle)", 
                  "GDP", 
                  "Median Country Age")) %>%
  kable() 
```
...
van Paridon J & Thompson B (2019). subs2vec:  Word embeddings from subtitles in 55 languages.
