...
# Language bias and other psychological measures
...
```{r}
## TO DO: 
# add country labels (do this in 01_tidy_other_psych_measure_data)
# do correct stats that accounts for language dependence
# add text.

OTHER_PATH <- here("data/other/tidy_other_psych_measures.csv")

other_data <- read_csv(OTHER_PATH) %>%
  select(country_code, lang_es_sub, genderdif_fh, self_efficacy_diff_sg) 

ggplot(other_data, aes(x = lang_es_sub, y = genderdif_fh, label = country_code)) +
...
```{r}
ggplot(other_data, aes(x = lang_es_sub, y = self_efficacy_diff_sg, label = country_code)) +
