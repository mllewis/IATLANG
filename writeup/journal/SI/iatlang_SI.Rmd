...
summary_stat <- all_es %>%
    select(-"Language IAT (Wikipedia, untranslated)", -n_participants) %>%
...
## Replication on untranslated corpora
Both the Subtitle and Wikipedia coprora likely contain some documents that are translated from other languages (e.g., the Wikipedia article on "Paris" is written in French and then translated into Dutch). The parallel content across languages allows us to estimate the gender bias in language statistics, while holding content constant across languages. Nevertheless, content may itself be a driver of gender bias (e.g. a language has more articles on physics topics relative to politicians). To understand the contribution of language-specific content on gender bias, we constructed a corpus of Wikipedia articles in each language that were originally written in the target language (untranslated). To identify untranslated articles, we did x.

trained models on these corpora, and conducted the same analyses as with the other corpora.
...
note excluedmandarn (n = 34)


```{r}
ggplot(all_es, aes(x = `Language IAT (Wikipedia, untranslated)`, 
                   y = `Residualized Implicit Bias (IAT)`)) +
  geom_point(alpha = .8, aes(size = log(n_participants))) +
  geom_smooth(method = "lm") +
  theme_classic()

```

Table below shows the correlation (Pearson’s r) for all measures in Study 1 and 2 at the level of languages, including estimates of language bias obtained from models trained on the untranslated Wikipedia corpus (highlighted in yellow).  Single asterisks indicate p < .05 and double asterisks indicate p < .01. The + symbol indicates a marginally significant p-value, p < .1.
```{r}
