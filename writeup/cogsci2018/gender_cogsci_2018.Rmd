...
SCALE_MIDPOINT <- mean(country_means_career_implicit$es_behavioral_iat, 
                       na.rm = T)

# Europe inset (note that Hong Kong is missing from this map)
p1 <- ggplot(map_data, aes(x = long, y = lat, 
                            group = group, 
...
p2 <- ggplot(country_means_career_implicit_with_index,
...
ggplot(map_data, aes(x = long, y = lat, 
                            ...
                            fill = es_behavioral_iat)) + 
  #scale_fill_gradient2("",# breaks =  c(.9, 1, 1.1, 1.2),
                       #limits=c(0, SCALE_MIDPOINT + .2), 
                 #      low = "white", high = "blue") +
  geom_polygon(color = "black", size = .1) +
  ggtitle("Implicit psychological gender bias effect size ") +
  scale_fill_viridis(option="viridis", limits=c(.85, 1.2), direction = -1, name="") +
  theme(text = element_text(size = TEXT_SIZE),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill = "white"),
...
df  = filter(map_data, !is.na(es_behavioral_iat)) %>%
  select(country_name, es_behavioral_iat) %>%
  distinct() 

ggplot(df, aes(es_behavioral_iat)) +
  geom_density(fill = "black") +
  geom_vline(aes(xintercept = 0), color = "red", linetype = 2) +
  ylab("") +
  xlab("IAT effect size") +
  xlim(-.1, 1.5) +
  theme_classic() +
...
We compared implicit and explicit gender biases to a measure of objective gender equality,
the Women, Peace, and Security Index (WPS, 2017). This metric describes the degree of inclusion, justice, and security of women by country, with larger values indicating higher gender equality. 
...
Figure 2 shows the effect size measures derived from the English Wikipedia corpus plotted against effect size estimates reported by CBN from two different models (trained on the Common Crawl and  Google News corpora). With the exception of biases related to race and age, effect sizes from the Wikipedia corpus are comparable to those reported by CBN. In particular, for the gender-career IAT---the bias relevant to our current purposes---we estimate the effect size to be  `r round(gender_bias_es_wiki,2)`, while CBN estimates it as approximately  `r round(mean(c(gender_bias_es_cc,gender_bias_es_gn)),2)`.
...
#write_csv(all_hand_es, "/Users/mollylewis/Documents/research/Projects/1_in_progress/IATLANG/analyses/2_pretrained_w2v_models/embedding_IAT_v2/behavioral_by_language.csv")
```


```{r behavior_vs_language_plot, fig.env="figure", fig.pos = "t", fig.align='center', fig.width=3.3, fig.height=3.1, set.cap.width=T, num.cols.cap=1, fig.cap = "Study 2b: Behavioral IAT gender bias from Study 1 presented by language,  versus language-embedding IAT gender bias. Language-embedding biases are estimated from models trained on each language."}
...
ggplot(all_hand_es, aes(x = es_hand_translation, y = es_behavioral_iat_weighted)) +
  ...
  geom_text_repel(aes(label = language_name), size = 2.4, box.padding = 0.4) + 
  geom_point(size = .7) +
...
es_with_gender <- all_hand_es %>% 
  left_join(gender_data, by = c("wiki_language_code" = "language_code")) %>%
  filter(!is.na(es_hand_translation))

ggplot(es_with_gender, 
       aes(x = es_hand_translation, y = es_behavioral_iat_weighted)) +
  geom_smooth(method = "lm", alpha = .1, , size = .9, color = "black") +
  geom_text_repel(aes(label = language_name.x, 
                      color = as.factor(wikipedia_grammar_type2)), size = 2.4, box.padding = 0.4) + 
  geom_point(size = .7, aes(color = as.factor(wikipedia_grammar_type2))) +
...
```{r, all_corrs_by_language}
wps_lang <-  wps %>%
  left_join(countries_to_langs %>%
              select(language_name, country_name)) %>%
  filter(!is.na(language_name)) %>%
  group_by(language_name) %>%
  summarize(mean_wps_index = mean(wps_index))

all_measures_df_by_language <- all_hand_es %>%
 # left_join(gender_data %>%
   #           select(language_code, wikipedia_grammar_type2),
   #         by = c("wiki_language_code" = "language_code")) %>%
  select(language_name, es_hand_translation, es_behavioral_iat_weighted) %>%
  left_join(language_means_career_explicit_language %>% ungroup() %>%
              select(language_name, exp_mean_diff_weighted)) %>%
  left_join(wps_lang)  %>%
  select(es_behavioral_iat_weighted,
           exp_mean_diff_weighted,
           mean_wps_index,
           es_hand_translation) %>%
  rename(`Behavioral IAT` = "es_behavioral_iat_weighted",
         `Explicit Bias` = "exp_mean_diff_weighted",
         `WPS Index` = "mean_wps_index",
         `Language IAT` = "es_hand_translation")

# test correlations
BL <- cor.test(all_measures_df_by_language$`Behavioral IAT`, all_measures_df_by_language$`Language IAT`)$p.value

BE <- cor.test(all_measures_df_by_language$`Behavioral IAT`, all_measures_df_by_language$`Explicit Bias`)$p.value

BW <- cor.test(all_measures_df_by_language$`Behavioral IAT`, all_measures_df_by_language$`WPS Index`)$p.value

LE <- cor.test(all_measures_df_by_language$`Language IAT`, all_measures_df_by_language$`Explicit Bias`)$p.value

LW <- cor.test(all_measures_df_by_language$`Language IAT`, all_measures_df_by_language$`WPS Index`)$p.value

EW <- cor.test(all_measures_df_by_language$`Explicit Bias`, all_measures_df_by_language$`WPS Index`)$p.value

all_corrs <- all_measures_df_by_language %>%
         correlate(use = "pairwise.complete.obs")   %>%
         shave() %>%
         fashion()

all_corrs[all_corrs$rowname == "Language IAT","Behavioral.IAT"] <- paste0(all_corrs[all_corrs$rowname == "Language IAT","Behavioral.IAT"], "*")

all_corrs[all_corrs$rowname == "WPS Index","Behavioral.IAT"] <- paste0(all_corrs[all_corrs$rowname == "WPS Index","Behavioral.IAT"], "*")

all_corrs[all_corrs$rowname == "Explicit Bias","Behavioral.IAT"] <- paste0(all_corrs[all_corrs$rowname == "Explicit Bias","Behavioral.IAT"], "  ")

# c(BL, BE, BW, LE, LW, EW)
```

```{r coding-table, results="asis", tab.env = "table", cache = F}
all_corrs_clean <- all_corrs %>%
  select(-Language.IAT) %>%
    rename(`Behavioral IAT` = "Behavioral.IAT",
          `Explicit Bias` = "Explicit.Bias",
          `WPS Index` = "WPS.Index",
           " " = "rowname")

measure_corr_table <- all_corrs_clean %>%
  xtable(caption = "Correlation (Pearson's r) for all measures in Studies 1 and 2. Asterisks indicate significance at the .05 level.",
          align = "rrrrr",  booktabs = T)

#print(measure_corr_table, type = "latex", comment = F, 
#      table.placement = "t",
...
xtable::print.xtable(measure_corr_table, 
                     type="latex", 
                     comment = F, 
                     table.placement = "t",
                     hline.after = hlines,
...
Table 1 presents the correlation between language IAT gender bias estimated from the native language embedding model and all other measures. Language IAT and Behavioral IAT effect sizes were positively correlated (_r_ = `r unlist(round(behavior_lang_cor_imp$estimate, 2), use.names = F)`; _p_ = `r unlist(round(behavior_lang_cor_imp$p.value, 2), use.names = F)`; Fig.\ 3). Explicit gender bias (_r_ = `r unlist(round(behavior_lang_cor_exp$estimate, 2), use.names = F)`; _p_ = `r unlist(round(behavior_lang_cor_exp$p.value, 2), use.names = F)`) and WPS index (_r_ = .25; _p_ =  .29) were not reliably correlated with language gender bias.
...
language-as-reflection hypotheses. In Study 3, we try to distinguish between the two hypotheses by asking whether there is a relationship between psychological gender bias and language along a linguistic dimension that is unlikely to be a subject of rapid change---namely, grammatical gender. While of course grammars do change over time, they are
...
included `r nrow(filter(gender_data, wikipedia_grammar_type2 == "Gender"))`  languages with  and `r nrow(filter(gender_data, wikipedia_grammar_type2 == "No Gender"))`  without grammatical gender.

## Results and Discussion

```{r grammatical_gender_plot, fig.env="figure", fig.pos = "t", fig.align='center', fig.width=3.5, fig.height=2.5, set.cap.width=T, num.cols.cap=1, fig.cap = "Study 3: Behavioral IAT gender bias from Study 1 as a function of whether participants' native language encodes gender grammatically. Each point corresponds to a language with outliers shown as triangles (jittered for visibility)."}
...
d_language_grammar <- mes(mean(mf$es_hand_translation, na.rm = T), 
                          mean(none$es_hand_translation, na.rm = T),
                          sd(mf$es_hand_translation, na.rm = T), 
                          sd(none$es_hand_translation, na.rm = T),
                          nrow(mf[!is.na(mf$es_hand_translation),]), 
                          nrow(none[!is.na(none$es_hand_translation),]), 
                        verbose = F)
...
Languages that encode grammatical gender tended to have speakers with greater psychological gender bias (Study 1; _M_ = `r round(mean(mf$es_behavioral_iat_weighted), 2)`; _SD_ =  `r round(sd(mf$es_behavioral_iat_weighted), 2)`) compared to speakers of languages that do not grammatically encode gender (_M_ = `r round(mean(none$es_behavioral_iat_weighted), 2)`; _SD_ =  `r round(sd(none$es_behavioral_iat_weighted), 2)`), though this difference was not reliable (_d_ = `r gender_grammar_d$d` [`r gender_grammar_d$l.d`, `r gender_grammar_d$u.d`], _t_(`r round(with_hindi_t$parameter,2)`) = `r round(with_hindi_t$statistic,2)`; _p_ = `r round(with_hindi_t$p.value, 2)`; Fig. 4). In a post-hoc analysis, we excluded outliers located more than two standard deviations from the group mean (Hungarian and Hindi). With these exclusions, we find a reliable difference between language types (_d_ = `r gender_grammar_d_no_hindi$d` [`r gender_grammar_d_no_hindi$l.d`, `r gender_grammar_d_no_hindi$u.d`]; _t_(`r round(no_hindi_t$parameter,2)`) = `r round(no_hindi_t$statistic,2)`; _p_ < .01). 

In addition, we find the same pattern for language IAT (Study 2), with languages that encode gender grammatically tending to have larger language IAT gender biases, compared to those that do not (_t_(`r round(t_language$parameter,2)`) = `r round(t_language$statistic[[1]],2)`; _p_ = `r round(t_language$p.value, 2)`). 
...
```{r save_complete_df_by_country, eval = F}
all_measures_df <- country_means_career_implicit_with_index %>% # implicit
  ...
  left_join(prop_women)
# write_csv(all_measures_df, "analysis/all/all_measures_df.csv")
```

\vspace{1em} \fbox{\parbox[b][][c]{7.3cm}{\centering All code and data for this project are available at\ \url{https://github.com/mllewis/IATLANG}}} \vspace{1em}
