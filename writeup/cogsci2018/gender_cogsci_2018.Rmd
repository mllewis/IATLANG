...
language_means_career_implicit <- read_csv("analysis/study2b/data/career_effect_sizes.csv")  # this comes from IAT_wiki_lang.R

d <- country_means_career_implicit %>%
  left_join(countries_to_langs, by = "country_name") %>%
  left_join(language_means_career_implicit, by = c("wiki_language_code" = "language_code" ))
# google translate does not have cantoness

langs_to_do <- c("ro", "zu", "th", "vi", "ga", "hi", "hu", "el", "hr", "ps")
```

TO DO:
- do effect seizes for IAT behavioral
- weight behavior and implict regression by certainty 
- deal with translations - google vs. non google (remove google ones from "hand translations")
