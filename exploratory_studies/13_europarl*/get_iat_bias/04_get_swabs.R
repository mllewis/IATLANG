...
MODEL_PATH_EURO <- here("exploratory_studies/13_europarl/get_iat_bias/europarl_subsetted_models/calculated/")
OUTFILE <- here("exploratory_studies/13_europarl/get_iat_bias/iat_swabs.csv")
NCLUSTERS <- 4
...
europarl_langs <- list.files(MODEL_PATH_EURO) %>%
  str_split("\\.|_") %>%
  map_chr(~.[3]) %>%
  data.frame() %>%
  rename(lang = ".") %>%
  mutate(path = paste0(MODEL_PATH_EURO, "europarl.untrans_"))

all_langs <- europarl_langs %>% 
  mutate(model = "europarl") %>%
  select(-model) %>%
  mutate(id = 1:n()) %>%
  nest(-id) 
