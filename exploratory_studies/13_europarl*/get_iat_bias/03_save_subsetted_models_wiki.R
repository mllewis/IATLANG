...
MODEL_PREFIX <- "/Users/mollylewis/Downloads/europarl_models/untrans_europarl_5ep_"
OUTMODEL_PREFIX <- here("exploratory_studies/13_europarl/get_iat_bias/europarl_subsetted_models/")
...
save_subsetted_model <- function(current_lang, trans_df, model_prefix, out_model_prefix){
  ...
  model_path <- paste0(model_prefix, current_lang, ".txt.vec")
  translation_status <- str_split(basename(model_path), "_")[[1]][1]
  ...
  # write raw vectors
  write_csv(relevant_vectors, paste0(out_model_prefix, "raw/europarl.", translation_status,
                                     "_", current_lang,   "_raw.csv"))
  ...
  write_csv(calculated_vectors, paste0(out_model_prefix, "calculated/europarl.", 
                                       translation_status,"_",
                                       current_lang,  "_calculated.csv"))
  
}

# get all subsetted models
walk(c("da", "de", "en", "es", "fi", "fr", "it", "nl", "pl", "pt", "ro", "sv"), 
