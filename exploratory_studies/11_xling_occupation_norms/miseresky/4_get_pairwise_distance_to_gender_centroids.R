...
CENTROID_PATH <- "gender_centroids_wiki_xling.csv"
OCCUPATION_TRANSLATIONS <- "miseresky_translations_clean.csv"
OUTFILE  <- "embedding_gender_bias_occupations.csv"
LANGS <- list("en", "it", "fr", "de", "no")

all_centroids <- read_csv(CENTROID_PATH)
all_translations <- read_csv(OCCUPATION_TRANSLATIONS) %>%
  mutate(target_word = English) %>%
  select(target_word, everything()) %>%
  select(-Slovak, -Czech) %>%
  gather("language_name","translation",  -1) %>%
  mutate(wiki_lang_code = case_when(language_name == "English" ~ "en",
                                   language_name == "Norwegian" ~ "no",
