...
process_one_session <- function(path_file_name){
  ...
  out_file_name <- paste0(dirname(path_file_name), "/tidy/", session_name, "_tidy.csv")
...
setdiff(total, complete) %>%
  ...
  unlist() %>%
  walk(possibly(process_one_session, NA))


complete <- list.files("/Volumes/wilbur_the_great/europarl/sessions/tidy", full.names = T)  %>%
  basename() %>%
