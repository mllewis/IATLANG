...
* GGI: World Economic Forum’s Gender Gap Index -  http://reports.weforum.org/global-gender-gap-report-2016/rankings/  (WEF_GGGR_Dataset_2016.xlsx)
...
bias_measures <- read_csv("../../data/IAT/Gender-Career/by_country_means_400.csv") %>%
  ...
  rename(career_IAT_mean = mean, 
         carreer_IAT_intercept_age_moderator = estimate_a_intercept, 
         carreer_IAT_intercept_age_and_sex_moderator = estimate_as_intercept)
...
```{r small_corr_plot1, fig.height = 8, fig.width = 8}
...
## Correlation between measures - language level

```{r, fig.height = 8, fig.width = 8}
# merge together
full_df_lang = bias_measures %>%
  left_join(all_gender_measures_transformed,  
            by = c("countryres" = "country_code")) %>%
  select(-country_name, -weapons_google) %>%
  group_by(wiki_language_code) %>%
  summarize_at(vars(career_IAT_mean:sigi_fam_log), mean) %>%
  select(wiki_language_code, everything()) %>%
  select(-career_hand)

corr_mat <- cor(full_df_lang[,-1], 
                use = "pairwise.complete.obs")

p.mat <- cor.mtest(full_df_lang[,-1], 
                  conf.level = (1-ALPHA),  
                  use = "pairwise.complete.obs")$p
...
```{r}
ggplot(full_df_lang, aes(x = career_google, y = career_IAT_mean)) +
  #geom_point() +
