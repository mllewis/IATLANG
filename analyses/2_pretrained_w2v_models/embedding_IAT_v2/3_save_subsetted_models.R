...
lang_key <- read_csv(LANGKEY) %>%
  ...
  select(language_name, wiki_language_code) 
...
save_subsetted_model <- function(current_lang, trans_df, model_prefix){
  ...
  # write raw vectors
  relevant_raw_vector_path <- "subsetted_models/raw/"
...
walk(unique(translations$wiki_language_code), save_subsetted_model, translations, MODEL_PREFIX)
