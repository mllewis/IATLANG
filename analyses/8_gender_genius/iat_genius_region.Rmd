...
by_participant_df_none <-  IAT_behavioral_tidy  %>%
  left_join(IAT_lang_restricted, by = "language_code") 

by_participant_df_all <-  IAT_behavioral_tidy  %>%
   left_join(IAT_lang_restricted, by = "language_code") %>%
   mutate(effect_size_restricted = scale(effect_size_restricted, scale = F),
          age = scale(age, scale = F))
   
   by_participant_df2 <-  IAT_behavioral_tidy  %>%
   left_join(IAT_lang_restricted, by = "language_code") %>%
...
lme4::lmer(iat_score~effect_size_restricted+gender + age + (1|country)  + (1|lang), 
           by_participant_df_none %>% filter(lang %in% targ_langs)) %>%
  summary()

lme4::lmer(iat_score~effect_size_restricted+gender + age + (1|country)  + (1|lang), 
           by_participant_df_all %>% filter(lang %in% targ_langs)) %>%
  summary()
