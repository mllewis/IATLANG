...
k = participant_df_full %>%
  mutate(subid = 1:n())
lme4::lmer(es_iat_sex_age_resid ~ career_hand + ggi + es_explicit_sex_age_implicit_resid + (1|countryres),
data = k) %>%
  summary()

lme4::lmer(overall_iat_D_score ~ career_hand   + (1|countryres),
data = k) %>%
  summary()

ggplot(participant_df_full, aes(x = age_log, y = overall_iat_D_score, color = countryres )) +
...
country_df_with_lang <- country_df %>%
  left_join(language_df %>% select(wiki_language_code, career_hand))

lm(per_women_stem ~  career_hand * wps +  wps*es_behavioral_explicit_resid1, 
...
lm(es_behavioral_iat  ~  career_hand*es_behavioral_explicit + wps  + participant_age  * es_behavioral_explicit,
data = language_df_with_ggi) %>%
  summary()
```

```{r}
lm(es_behavioral_iat_resid1  ~  career_hand  + wps,
data = language_df_with_ggi) %>%
  summary()

ggplot(language_df_with_ggi, aes(x = career_hand, y = es_behavioral_iat)) +
  geom_point(aes(color =participant_age )) +
